...
## Quy tắc Tránh Hardcode (No Hardcoding)
- **TUYỆT ĐỐI KHÔNG** hardcode các chuỗi văn bản (text) hiển thị trên giao diện (UI) hoặc các thông báo trả về (messages). Bắt buộc phải sử dụng cơ chế đa ngôn ngữ (i18n): dùng hàm `t()` (Frontend) hoặc hàm `TsoMessageUtil.getMessage()` (Backend) thông qua file properties/JSON để hỗ trợ đa ngôn ngữ.
- **TUYỆT ĐỐI KHÔNG** hardcode các giá trị cấu hình như: đường dẫn thư mục (file paths), URL, API keys, credentials, port, v.v.
- Luôn sử dụng các biến cấu hình môi trường (ví dụ: `application.properties`, `.env`, `process.env`) để lưu trữ các giá trị này.
